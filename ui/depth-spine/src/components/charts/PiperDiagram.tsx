import type { Percentages } from '../../domain/quality';

/**
 * Trilinear Piper plot. Cation and anion triangles with the sample projected
 * into the diamond — the standard way to say what kind of water this is.
 *
 * The construction only works if the gap between the triangles equals their
 * side length: that is what puts the diamond's lower vertex exactly at apex
 * height and makes its edges parallel to the triangles' inner edges.
 */

const T = 90; // triangle side, and therefore the gap between them
...
/** Intersection of two parametric lines — the diamond projection. */
...
/** Grid at 20 % intervals: the three families of parallel lines. */
...
export function PiperDiagram({ p }: { p: Percentages }) {
  const cation: P = {
    x: X0 + (p.naK + p.mg / 2) * T,
    y: BASE_Y - p.mg * T * H,
  };
  ...
  return (
    ...
    >
      ...
      <polygon
        points={diamond.map((d) => `${d.x},${d.y}`).join(' ')}
        fill="none"
        stroke={edge}
      />
      ...
      <g fontFamily="'IBM Plex Mono', monospace" fontSize={8.5} fill={label}>
        ...
        <text x={CX - T / 2 - 5} y={bottom.y - DIAMOND_H / 2 + 3} textAnchor="end">
          Ca+Mg
        </text>
        <text x={CX + T / 2 + 5} y={bottom.y - DIAMOND_H / 2 + 3}>
          Cl+SO₄
        </text>
      </g>
    </svg>
