...
produced it. Eight areas were audited in parallel by independent auditors;
seven reported, and every finding below was reproduced by the auditor that
reported it, with a script or by dumping the figure or document. The eighth
area (bundled data, geodesy and parsers) is being audited separately and
will be appended when it reports. A few findings carry an independent
second verification or a direct check by the maintainer of this branch,
...
8. Data, geodesy and parsers. Appended when its audit reports.
...
- [ ] **C ves-1. A conductive half-space is reported as a water zone down
...
- [ ] **C ves-2. Any conductive bottom layer is labelled "fractured bedrock,
...
- [ ] **H ves-3. The ranking prefers B (2) over A (1) on 2.7 ohm-m of
...
- [ ] **H ves-4. A misfit above the 10 percent target is never flagged.**
...
- [ ] **H ves-5. Overlapping readings that disagree by 45-98 percent at the
  ...
  the field problem is invisible to the geophysicist.
- [ ] **M ves-6. "Resolves a 3 layer subsurface" when the first boundary is
  x/3.7 unresolved** and a refit without it costs two points of a 13
  percent misfit.
- [ ] **M ves-7. The original IPI2Win models are imported and discarded.**
  No comparison reaches the report, and the toolkit cannot reproduce their
  reported ERR (35.8 vs 21.5 percent, 96.7 vs 26.5 percent).
- [ ] **M ves-8 / maps-survey-4. Four different depth-of-investigation
  ...
  layer column beside it to 40 m.
- [ ] **M ves-9 / reports-12. The preference table column "Apparent
...
Being audited; appended when it reports. Known so far: the consistency
checker's district test uses overlapping bounding boxes and can report a
point in two districts ("falls in Western Area Rural, Moyamba") although
the real polygons are bundled; the WHO table's treatment of total coliforms
(reports-9). The UTM arithmetic was verified correct.
